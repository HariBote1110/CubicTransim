...
- **JS→wasm の引数コピー**: `setInstances`/`uploadMeshChunk` はグルーコードが引数ごとに
  malloc+コピーする(`uploadMeshChunk` は1呼び出しにつき4回)。wasm 側に常設の
  ステージングを持たせ、JS が `Float32Array(memory.buffer, ptr, len)` へ直接書いて
  オフセットだけ渡す形にすれば消せる。TS 側の API 変更を伴う。

...
## 追記(0.5.0-Alpha-9a): 旧セーブの地下ランプdir逆転を正規化

91f5945(0.5.0-Alpha-8f以前)でramp.dirの契約(「高さの高い側=地表側を指す」)を
修正したが、それより前に作られたセーブは地下(`ramp.base<0`)のランプのdirが
逆(地下側)を向いたまま保存されており、`buildRampTrackParts`がdirを高い側として
解釈するため線路が寸断・ズレて描画される問題が残っていた。

セル単体では新旧どちらの形式か判別できない(dirはただの方向ビットで、いつ作られた
セーブかという情報を持たない)ため、`sim/persistence.ts`の
`normaliseUndergroundRampDirs`が隣接セルの接続状態から構造的に判定する:
「dir/oppositeのどちらが、実際にbase+1レベル(高い側)へ接続しているか」を、
base+1が地平(0)なら通常のconnections、地下ならuppers[base+1].connectionsの
戻りビットで確認する。opposite側だけが一致すればdirが逆(旧形式)と判定して反転、
両方/どちらも一致しない曖昧なケースは判別不能なので変更しない。

`deserialiseWorld`のrailMap構築直後に必ず適用する(新形式のセーブに対しては
参照・内容とも不変)。Vitestで旧形式補正・新形式不変・曖昧ケース不変・
非対象セル(base>=0/rampなし)不変の4パターンを固定した
(`src/sim/persistence.test.ts`)。

...
## 追記(v0.3.0-Alpha-18a: 坂の桁寄り半分(level2)に支えが無い不具合を修正)

ユーザーから「高架と地上線の繋がり(坂)の描画が若干怪しい」との指摘を受けて
`render/trackGeometry.ts`・`components/TrackNetwork.tsx`を再点検した。

### 根本原因
`TrackNetwork.tsx`の坂セル支持構造の分岐が、
`if (base===0 && level===1) 土盛りくさび / else if (base>0 && level===1) 支柱`
という**level===1のケースしか網羅していなかった**。坂は必ずlevel1(base寄り
半分)とlevel2(桁寄り半分)の2セルで構成される(`RAMP_BOUNDARY_*`参照)が、
level2側はどの条件にも一致せず、`base`の値によらず**支えの無い浮遊状態**の
ままだった。level2は定義上必ず`rampHeightAtPos(posLow, base) > 0`(地面に
接しない)なので、本来は常に支柱(`buildRampPierPart`)が必要なケースだった。
このためユーザーの言う「若干怪しい」見た目は、坂の桁寄り半分が宙に浮いて
見えることだったと考えられる(短い坂ではデッキ側の桁の橋脚と視覚的に
近接して目立ちにくいが、レベル差が大きい坂ほど明確な空白区間になる)。

### 修正
- 分岐を`if (base===0 && level===1) 土盛りくさび / else 支柱`に単純化。
  「地平(base=0)に接するlevel1側だけが土盛り、それ以外(base>=1のlevel1、
  および常にlevel2)は支柱」という設計意図どおりの網羅になった。

### 検証
- Vitest 500件、`npm run build`(tsc -b && vite build)ともに通過。
- ブラウザ実機検証: Lv1の浮いた高架(x=0〜7)を建設し、地平の線路(x=10〜7)を
  端タイルへ接続して坂(x=8がlevel2、x=9がlevel1)を作成。修正前は同じ
  railMap状態でx=8(level2)の区間の下に支柱が無く、修正後は坂の全区間に
  沿って支柱が並び、桁から地面まで視覚的に連続した構造になることを
  スクリーンショットで確認した。

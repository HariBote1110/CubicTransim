...
const train = (id: string, schedule: string[], serviceId?: string): TrainData => ({
  id, x: 0, z: 0, schedule, scheduleIndex: 0, status: 'running', cars: 2, serviceId,
});

/** id をそのまま路線idにも種別idにも使う便宜ヘルパー(各停種別1つだけの路線)。 */
const lineAndService = (id: string, stops: string[]): { line: LineData; service: ServiceData } => ({
  line: { id, name: id, stops, colour: '#fff' },
  service: { id, lineId: id, name: '各停', skipStationIds: [], headwaySeconds: 0 },
});

const graphOf = (trains: TrainData[], pairs: { line: LineData; service: ServiceData }[]) =>
  buildServiceGraph(trains, pairs.map(p => p.line), pairs.map(p => p.service));
...
describe('サービス網の構築', () => {
  ...
  it('種別に所属する列車は路線の共有運行表を使う', () => {
    const trains = [train('t1', ['X'], 'g1')];
    const graph = graphOf(trains, [lineAndService('g1', ['A', 'B'])]);
    expect(graph.get('A')?.map(e => e.to)).toEqual(['B']);
    // 列車自身の運行表(['X'])は使われない
    expect(graph.has('X')).toBe(false);
  });

  it('同じ種別の列車が複数いても辺は重複しない', () => {
  ...
  it('辺には由来する系統(lineId=serviceId)が入る', () => {
    const graph = graphOf([train('t1', ['A', 'B'], 'g1')], [lineAndService('g1', ['A', 'B'])]);
...
describe('経路探索', () => {
  ...
  it('乗換が必要なら乗換駅で区切った複数のlegになる', () => {
    ...
    const pairs = [lineAndService('g1', ['A', 'B']), lineAndService('g2', ['B', 'C'])];
    const route = findRoute(graphOf(trains, pairs), 'A', 'C');
  ...
  it('乗換の少ない経路を、遠回りでも優先する', () => {
    ...
    const pairs = [
      lineAndService('g1', ['A', 'B', 'C', 'D']),
      lineAndService('g2', ['A', 'X']),
      lineAndService('g3', ['X', 'D']),
    ];
    const trains = [train('t1', [], 'g1'), train('t2', [], 'g2'), train('t3', [], 'g3')];
    const route = findRoute(graphOf(trains, pairs), 'A', 'D');
  ...
  it('乗換回数の上限を超える経路は採用しない', () => {
    ...
    const pairs = [
      lineAndService('g1', ['A', 'B']), lineAndService('g2', ['B', 'C']),
      lineAndService('g3', ['C', 'D']), lineAndService('g4', ['D', 'E']),
    ];
    const trains = pairs.map((p, i) => train(`t${i}`, [], p.service.id));
    const graph = graphOf(trains, pairs);

...
const PlatformSide: React.FC<{ side: 1 | -1; doors: PlatformDoorType; isEnd: boolean }> = ({
  ...
  return (
    <group>
      {/* ホーム床 */}
      <mesh position={[cx, PLATFORM_HEIGHT / 2, 0]} receiveShadow castShadow material={MATERIALS.platform}>
        <boxGeometry args={[PLATFORM_WIDTH, PLATFORM_HEIGHT, 1.0]} />
      </mesh>
      {/* 軌道側の側壁(床とわずかに色を変えて厚みを見せる) */}
      <mesh position={[side * (PLATFORM_INNER + 0.012), PLATFORM_HEIGHT / 2 - 0.01, 0]} material={MATERIALS.platformSide}>
        <boxGeometry args={[0.024, PLATFORM_HEIGHT - 0.02, 1.0]} />
      </mesh>
      {/* 点字ブロック(黄色い警戒線) */}
      <mesh position={[edgeX, PLATFORM_HEIGHT + 0.005, 0]} material={MATERIALS.tactile}>
        <boxGeometry args={[TACTILE_WIDTH, 0.012, 1.0]} />
      </mesh>

      {/* 上屋。ホームの外寄りだけを覆う細い庇にして、軌道側(点字ブロック・停車中の
          列車)が真上から見ても隠れないようにする。 */}
      <mesh position={[side * (PLATFORM_OUTER - CANOPY_WIDTH / 2 + 0.02), CANOPY_HEIGHT, 0]} castShadow material={MATERIALS.canopyRoof}>
      ...
      {doors !== 'none' && [-0.32, 0.32].map((z, i) => (
        <group key={i} position={[doorX, PLATFORM_HEIGHT + doorHeight / 2, z]}>
          <mesh material={MATERIALS.platformDoor}>
            <boxGeometry args={[0.05, doorHeight, 0.34]} />
          </mesh>
          <mesh position={[side * 0.028, 0.01, 0]} material={MATERIALS.platformDoorGlass}>

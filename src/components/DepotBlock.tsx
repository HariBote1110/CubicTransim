 ...
 */
// 車庫の外観メッシュは装飾であり選択対象ではない(購入クリックは地面プレーンの
// レイキャストで判定するため)。地面クリックを奪わないようレイキャストを外す。
const noRaycast = () => null;

export const DepotBlock: React.FC<Props> = ({ position, rotation = 0 }) => (
  <group position={position} rotation={[0, rotation, 0]}>
    {/* 土台(コンクリート床) */}
    <mesh position={[0, 0.04, 0]} receiveShadow raycast={noRaycast}>
      <boxGeometry args={[0.96, 0.08, 0.96]} />
      <meshStandardMaterial color="#9a978f" roughness={1} />
    </mesh>
    {/* 庫内の線路(入口から奥へ) */}
    {[-0.125, 0.125].map(x => (
      <mesh key={x} position={[x, 0.11, 0]} raycast={noRaycast}>
    ...
    {/* 側壁 */}
    {[-1, 1].map(side => (
      <mesh key={side} position={[side * 0.42, 0.36, 0.03]} castShadow receiveShadow raycast={noRaycast}>
        <boxGeometry args={[0.08, 0.62, 0.9]} />
        <meshStandardMaterial color={PALETTE.depotWall} roughness={0.85} />
      </mesh>
    ))}
    {/* 奥の妻壁 */}
    <mesh position={[0, 0.36, 0.44]} castShadow raycast={noRaycast}>
      <boxGeometry args={[0.92, 0.62, 0.08]} />
      <meshStandardMaterial color={PALETTE.depotWall} roughness={0.85} />
    </mesh>
    {/* 入口側の垂れ壁(シャッターの上端。開口を残して車両が入れるようにする) */}
    <mesh position={[0, 0.62, -0.44]} castShadow raycast={noRaycast}>
      <boxGeometry args={[0.92, 0.12, 0.08]} />
      <meshStandardMaterial color={PALETTE.depotWall} roughness={0.85} />
    </mesh>
    {/* 開口の暗がり */}
    <mesh position={[0, 0.3, -0.42]} raycast={noRaycast}>
      <boxGeometry args={[0.7, 0.5, 0.02]} />
      <meshStandardMaterial color="#161b21" roughness={1} />
    </mesh>

    {/* 片流れ屋根(入口側が低い) */}
    <mesh position={[0, 0.71, 0]} rotation={[-0.06, 0, 0]} castShadow raycast={noRaycast}>

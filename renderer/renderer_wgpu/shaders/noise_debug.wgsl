struct Params { seed:u32, half_extent:i32, count:u32, _pad:u32 };
...
fn main(@builtin(global_invocation_id) gid:vec3<u32>){
 ...
 let q0=value_noise_fixed(derive_octave_seed(params.seed,0u),p.x,p.y,40); let q1=value_noise_fixed(derive_octave_seed(params.seed,1u),p.x,p.y,20);
 let q2=value_noise_fixed(derive_octave_seed(params.seed,2u),p.x,p.y,10); let q3=value_noise_fixed(derive_octave_seed(params.seed,3u),p.x,p.y,5); let n=composite_num(params.seed,p.x,p.y);
 let sx40=smooth_q(p.x,40); let sz40=smooth_q(p.y,40); output[0]=n.hi;output[1]=n.lo;output[2]=0u;output[3]=q0;output[4]=0u;output[5]=q1;output[6]=0u;output[7]=q2;output[8]=0u;output[9]=q3;output[10]=sx40.x;output[11]=sx40.y;output[12]=sz40.x;output[13]=sz40.y;
}
